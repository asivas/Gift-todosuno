...
const InactiveUsers = () => {
  ...
  const [ascendido, setAscendido] = useState(false);
  ...
  const ascenderNivel = async () => {
    if (
      ...
      dataUser.username !== "Nelson" &&
      dataUser.username !== "Escro"
    ) {
  ...
  return (
    <div className="dashboard__top">
      ...
      {ascendido ? (
        <button className="addToRefer" onClick={ascenderNivel}>
          Subir de nivel
        </button>
      ) : (
        <p style={{ display: "none" }}></p>
      ...
      {fatherComplete ? (
        <p style={{ color: "Red", fontWeight: "300", fontSize: "15px" }}>
          Tu padre tiene que aceptarte en su cuadro.
        </p>
      ) : (
        <p style={{ display: "none" }}></p>
      )}
